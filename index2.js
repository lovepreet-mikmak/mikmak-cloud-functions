 ...
 */
const getJobs = ({n,options}, callback) =>{
   console.log("n is---", n);
...
const extractCSVJob = async (bucketName = "", fileName = "", dataSet = "", table = "") => {
    ...
    async.times(batchs, function(n, next) {
        const query =  `SELECT *From ${dataSet}.${table} ORDER BY id LIMIT ${batchSize} OFFSET ${batchSize*(n+1)}`;
        const options = {
            query: query,
            location: 'US',
        }
        getJobs({n,options}, function(err, res) {
            next(err, res);
        });
    }, function(err, result) {
